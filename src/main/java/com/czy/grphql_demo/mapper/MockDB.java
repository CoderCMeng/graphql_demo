...
public class MockDB {
    ...
    public static List<Address> queryAddressListByUserIds(Collection<String> userIdList) {
        log.info("MockDB 执行queryAddressListByUserIds：userIdList = {}", userIdList);
        return ADDRESSES.stream().filter(a -> userIdList.contains(a.getUserId())).collect(Collectors.toList());
    }
}
